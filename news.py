...
def getnews(value):
    try:
        ...
        soup=bs(r,'lxml')
        x=soup.find_all('div',class_="pt-cv-ifiel")
        val={}
        val["data"]=[]
        for i in range(0,len(x)):
            dic={}
            dic["title"]=x[i].find('h4',class_="news-card-image").text
            dic["images"]=x[i].find('img', class_="pt-cv-thumbnail")['style'].split("url(")[1].split(")")[0].replace("'",'')
            dic["news-link"]=x[i].find('a',class_='pt-cv-thumb-default')['style'].split("url(")[1].split(")")[0].replace("'",'')
        val["data"].append(dic)
        val['category']=value
